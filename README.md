...
Для решения задачи используется класс Human. В классе есть 2 поля: name и height. Также есть конструктор с параметрами, геттеры и сеттеры для каждого из полей, а так же метод возвращения строки. В классе main создается 3 человека с именами Клеопатра, Пушкин и Владимир и ростом 152, 167 и 189 соответственно. Кажде имя с ростом выводится на экран.
